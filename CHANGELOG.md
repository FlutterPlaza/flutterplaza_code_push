...
- **fix**: the iOS-side format guard in 0.1.9 accepted only ELF (`7F 45 4C 46`), based on the incorrect assumption that the code-push engine's `loadDynamicModule` on iOS wanted an `app-aot-elf` blob. It actually wants a **Mach-O 64-bit dylib** — iOS/macOS targets use `gen_snapshot --snapshot_kind=app-aot-macho-dylib`, which emits `CF FA ED FE` (little-endian, arm64/x86_64) or `FE ED FA CF` (big-endian). 0.1.10 inverts the guard: on iOS, Mach-O 64-bit is the required format, and **ELF is now explicitly rejected with a diagnostic pointing at the `flutter_compile 0.19.14+` upgrade**. Historical wrong-formats (Mach-O of the baseline binary via App.framework/App, raw Dart kernel, generic unknown magic) continue to be diagnosed with their specific cause + the required CLI upgrade.
- On any format mismatch, `CodePush.status` now reports the specific wrong format (ELF / kernel / unknown) and the SDK calls `_iosImmediateRollback` to delete `patch.vmcode` and siblings right away — no waiting for the three-strike auto-rollback to fire.
- Pairs with `flutter_compile 0.19.14` which fixes the CLI side by targeting `app-aot-macho-dylib` via the new `--target ios` flag on `fcp-tool snapshot`, producing the correct Mach-O dylib output.

## 0.1.9

- **fix**: load-time format guard rewritten. The Dart VM's `DN_Internal_loadDynamicModule` (reached via `ui.codePushLoadModule` → `Isolate.loadDynamicModule`) accepts exactly one payload format: an AOT dynamic-module **ELF** blob (magic `7F 45 4C 46`). Previous SDK versions rejected ELF on iOS based on the (incorrect) assumption that iOS patches needed to be raw Dart kernel or bytecode; they only accepted kernel as "valid", which aborted the VM because kernel is the *input* to the snapshot step, not the output the loader expects. 0.1.9 inverts the guard: ELF is now the required format, and raw kernel (`90 AB CD EF`) is explicitly rejected with an actionable diagnostic pointing at the `flutter_compile 0.19.13+` upgrade.
- The existing Mach-O rejection from 0.1.8 is preserved and now lives alongside the kernel rejection in a single combined format-guard block. Unknown magic bytes get a generic "expected ELF dynamic-module snapshot" error.
- On any format mismatch the SDK calls `_iosImmediateRollback` to delete `patch.vmcode` and siblings right away, so the next launch starts clean without waiting for the three-strike auto-rollback. `CodePush.status` carries a one-line diagnosis of the specific wrong format (Mach-O / kernel / unknown) so apps that show the debug overlay can surface it immediately.
- Pairs with `flutter_compile 0.19.13`, which inserts a `fcp-tool snapshot` step between `prepare` and `package` so freshly-built iOS patches finally ship the ELF blob the loader wants.

## 0.1.8

- **defensive fix**: reject Mach-O payloads explicitly in `checkAndInstall` before calling `ui.codePushLoadModule`. This guards against the `flutter_compile` bug (fixed in 0.19.11, see that package's CHANGELOG) where iOS patches were packaged as the baseline's `App.framework/App` Mach-O binary instead of the Dart kernel file the VM actually expects. Passing a Mach-O to `loadDynamicModule` aborts the VM with `SIGABRT` inside `DN_Internal_loadDynamicModule` and no recoverable error. Devices running 0.1.8 against an older `flutter_compile` CLI now get a clear status message (`Patch is Mach-O (magic <hex>) — expected Dart kernel. Upgrade flutter_compile to 0.19.11+ and rebuild the patch.`) and an immediate rollback instead of a crash loop. Complements the existing ELF guard — the load path now rejects both wrong formats up front.
